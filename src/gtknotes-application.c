...
GtknotesApplication *gtknotes_application_new(gchar *application_id, GApplicationFlags flags) {
  return g_object_new(GTKNOTES_TYPE_APPLICATION, "application-id", application_id, "flags", flags, NULL);
}
...
static void gtknotes_application_class_init(GtknotesApplicationClass *klass) {
  ...
  // Uused to create a window when the app is launched. The callback also notifies us
  // when the user launches another instance, so we can just present an existing window
  app_class->activate = gtknotes_application_activate;
}

static void gtknotes_application_show_about(G_GNUC_UNUSED GSimpleAction *action, G_GNUC_UNUSED GVariant *parameter, gpointer user_data) {
  GtknotesApplication *self = GTKNOTES_APPLICATION(user_data);
  g_return_if_fail(GTKNOTES_IS_APPLICATION(self));

  GtkWindow *window = gtk_application_get_active_window(GTK_APPLICATION(self));
  gtk_show_about_dialog(window, "program-name", "dNotes", "authors", "DoodlesEic", "version", "0.1.0", NULL);
}
...
static void gtknotes_application_init(GtknotesApplication *self) {
  ...
  g_signal_connect(about_action, "activate", G_CALLBACK(gtknotes_application_show_about), self);
